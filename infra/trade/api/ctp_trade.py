...
class _CtpTradeSpi(tapi.CThostFtdcTraderSpi):
    ...
    def OnRspAuthenticate(
        ...
        if not pRspInfo.ErrorID:
            ...
            logger.warning(f"Recv RspAuthenticate Err: {pRspInfo.ErrorID} - {pRspInfo.ErrorMsg}")
    ...
    def OnRspUserLogin(
        ...
        if not pRspInfo.ErrorID:
            ...
            logger.warning(f"Recv Login Err: {pRspInfo.ErrorID} - {pRspInfo.ErrorMsg}")
    ...
    def OnRspUserLogout(
        ...
        logger.warning(f"OnRspUserLogout: {pUserLogout.BrokerID} - {pUserLogout.UserID}")
    ...
    def OnRtnOrder(self, pOrder: 'CThostFtdcOrderField') -> "void":
        logger.info(
            f"OnRtnOrder {pOrder.OrderRef} TC-{pOrder.TimeCondition} "
            f"Status: {pOrder.OrderStatus} ({pOrder.OrderSubmitStatus}) - {pOrder.StatusMsg} "
            f"{pOrder.InstrumentID}@{pOrder.LimitPrice} | "
            f"V{pOrder.VolumeTotalOriginal}({pOrder.VolumeTraded} - {pOrder.VolumeTotal}) D{pOrder.Direction}"
        )
    ...
    def OnRtnTrade(self, pTrade: 'CThostFtdcTradeField') -> "void":
        logger.info(
            f"OnRtnTrade {pTrade.OrderRef} "
            f"{pTrade.InstrumentID}@{pTrade.Price} | V{pTrade.Volume} D{pTrade.Direction} | "
            f"{pTrade.TradeDate} {pTrade.TradeTime}"
        )
        ...
        # 累计统计该单号下的成交量
        v_prev = self.trade_vol_cnt.get(pTrade.OrderRef, 0)
        v_prev += pTrade.Volume
        self.trade_vol_cnt[pTrade.OrderRef] = v_prev
...
class CtpTrade:
    def __init__(
        ...
        self.trade_api = tapi.CThostFtdcTraderApi_CreateFtdcTraderApi(
            os.path.join(
                self.pth,
                os.sep.join(["infra", "trade", "api", "temp", "TRADE"])
            )
        )
    ...
    def req_id(self):
        ...
        order_id = self.trade_rule.request_id_generator()
    ...
    def __enter__(self):
        ...
        while not self.trade_spi.status:
            ...
            sleep(5)                        # 等待5秒，避免刚登陆就有交易时，查询持仓直接报错
            # self.query_position()         # 查询持仓需要等待一会儿，否则会报错
            self.query_account()
